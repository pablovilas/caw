...
impl TrayState {
    pub fn new() -> Self {
        ...
        let (menu, live) = build_menu(&[], &HashMap::new(), GroupBy::Project);
...
fn build_menu(
    ...
    _old_pid_map: &HashMap<String, u32>,
    group_by: GroupBy,
) -> (Menu, LiveMenu) {
    ...
    if sessions.is_empty() {
        ...
        let item = MenuItem::with_id(MenuId::new("summary"), &summary_text, false, None);
    ...
    let check = |g: GroupBy| if g == group_by { "  \u{2713}" } else { "" };
    ...
    let _ = menu.append(&PredefinedMenuItem::separator());
    let quit = MenuItem::with_id(MenuId::new("quit"), "Quit caw", true, None);
